...
class TableInfusing(nn.Module):
    ...
    def forward(self, seqs_in, table_in, table_scatters, lookups, line_nos, fields, indexes, bbox_data):
        enc_inp = self.encode(table_in, lookups, line_nos, fields, indexes, bbox_data)
        logits = self.decode(seqs_in, enc_inp, table_scatters)

        return logits

    def encode(self, table_in, lookups, line_nos, fields, indexes, bbox_data):
        ...
        bbox_data = self.embed(bbox_data)

        cell_emb = field_emb + line_no_emb + word_emb + bbox_data
