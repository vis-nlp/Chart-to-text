...
class Dataloader(object):
    def __init__(self, train_name, val_name, test_name, train_bbox_name, valid_bbox_name, test_bbox_name):
        if train_name:
        ...
        if train_bbox_name:
            with open(train_bbox_name, 'r', encoding='utf-8') as file:
                self.train_bbox = file.readlines()
        
        
        if valid_bbox_name:
            with open(valid_bbox_name, 'r', encoding='utf-8') as file:
                self.valid_bbox = file.readlines()
        
        if test_bbox_name:
            with open(test_bbox_name, 'r', encoding='utf-8') as file:
                self.test_bbox = file.readlines()
...
class NormalTableDatabase(Dataloader):
    def __init__(self, train_name, val_name, test_name, train_bbox_name, valid_bbox_name, test_bbox_name, batch_size=5, max_len=800):
        super(NormalTableDatabase, self).__init__(train_name, val_name, test_name, train_bbox_name, valid_bbox_name, test_bbox_name)
        self.batch_size = batch_size
        self.max_len = max_len
    ...
    def get_data(self, idx, option='train', debug=False, with_fields=False):
        ...
        for e in entry:
            ...
            bbox_data_line.extend([self.vocab.get(_, self.vocab['<UNK>']) for _ in bbox.split()])
        ...
        for i in range(len(table_in)):
            ...
            bbox_data[i] += (length - len(bbox_data[i])) * [self.vocab['<PAD>']]
            bbox_data[i] = bbox_data[i][:length]
        table_in = torch.LongTensor(table_in)
        table_scatters = torch.LongTensor(table_scatters)
        bbox_data = torch.LongTensor(bbox_data)
