...
def openData(dataPath):
    if not os.path.isfile(dataPath):
        df=pd.DataFrame({'0':['empty'],'1':['empty']})
    else:
...
def cleanAxisLabel(label):
    cleanLabel = re.sub('\s', '_', str(label))
...
dataset_dir = "../../../../c2t_dataset_statista/"
mapping_file = dataset_dir + 'val_index_mapping.csv'
...
for _file in range(len(dataFiles)):
    dataPath = dataset_dir + 'dataset/data_auto_extracted/' + dataFiles[_file]
    ...
    if axisTypes[axis].lower() == 'ordinal' or axisTypes[axis].lower() == 'numerical':
        if is_number(token) and are_numbers(valueArr):
            if float(words) == max([float(i) for i in valueArr]):
                ...
                return [1, f'templateValue[{axis}][max]']
            elif float(words) == min([float(i) for i in valueArr]):
                checkForParallelMultiColumn(axis, 'min', arrayIndex)
                return [1, f'templateValue[{axis}][min]']
    if words == valueArr[len(valueArr) - 1]:
        return [1, f'templateValue[{axis}][last]']
...
def openMultiColumnData(dataPath):
    ...
    cols = df.columns
    size = df.shape[0]
    chartType = getChartType(str(cols[0]))
...
for _file in range(len(dataFiles)):
    dataPath = dataset_dir + 'dataset/multiColumn/data_auto_extracted/' + dataFiles[_file]
